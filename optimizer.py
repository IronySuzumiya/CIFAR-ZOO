...
class PruneSGD(NameOptimizer):
    def __init__(self, params, lr=required, momentum=0, dampening=0,
                 weight_decay=0, nesterov=False, signed=False):
        ...
        self.prune_step = self.prune_step_signed if signed else self.prune_step_unsigned
    ...
    def __setstate__(self, state):
        super(PruneSGD, self).__setstate__(state)
        for group in self.param_groups:
    ...
    @torch.no_grad()
    def prune_step_unsigned(self, mask, closure=None):
        """Performs a single optimization step.
    ...
    @torch.no_grad()
    def prune_step_signed(self, mask, closure=None):
        """Performs a single optimization step.
        ...
        for group in self.param_groups:
            ...
            for name, p in group['params']:
                ...
                if name.split('.')[-1] == "weight" and len(p.shape) == 4:
                    p.add_(d_p.mul(mask[name]).abs().mul(mask[name]), alpha=-group['lr'])
                else:
                    p.add_(d_p, alpha=-group['lr'])
